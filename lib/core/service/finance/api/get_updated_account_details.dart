...
import '../../my_account/model/hub_manager_details_response.dart';

class UpdatedHubManagerDetails {
  Future<CommanResponse> getUpdatedAccountDetails() async {
    ...
    if (await Helper.isNetworkAvailable()) {
      ...
      if (hubManagerDetails.message.message == "success") {
        ...
        double offLineCashBalance =
            await DbSaleOrder().getOfflineOrderCashBalance();

        // print("OFFLINE CASH BALANCE $offLineCashBalance");
        // print("ONLINE CASH BALANCE ${hubManagerData.balance}");
        var newCashbalance =
            hubManagerData.balance.toDouble() + offLineCashBalance;
